...
// Function to format date from DD/MM/YYYY (database, now VARCHAR) to DD/MM/YYYY (display)
function formatDateForDisplay($dateString) {
    // If the database stores DD/MM/YYYY in a VARCHAR, we just return it.
    // If it ever gets a YYYY-MM-DD for some reason (e.g., old data),
    // we convert it to DD/MM/YYYY.
...
// Function to ensure date from input (DD/MM/YYYY) is ready for database (DD/MM/YYYY VARCHAR)
function formatDateForDatabase($dateString) {
    if (empty($dateString)) {
        return '';
    }
    // No conversion needed, as input is DD/MM/YYYY and DB wants DD/MM/YYYY (VARCHAR)
    return $dateString; 
}
...
if (isset($_POST['update_appointment'])) {
    if (isset($_POST['appointment_id']) && !empty($_POST['appointment_id'])) {
        ...
        // DATES ARE SENT AS DD/MM/YYYY from input.
        // We will directly use this format as the DB column is VARCHAR and we want DD/MM/YYYY.
        $dates_raw = $_POST['dates'];
        $dates = mysqli_real_escape_string($con, formatDateForDatabase($dates_raw)); // Now formatDateForDatabase just passes it through
...
if ($appointment_id_to_fetch) {
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        // This will now take the DD/MM/YYYY string from DB and display it as is.
        // It also handles potential old YYYY-MM-DD values if they exist.
